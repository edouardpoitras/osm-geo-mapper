...
pub fn get_amenity_geo_tile(props: &GeoTileProperties, geometry: Geometry) -> GeoTile {
    ...
    geotile_from_properties!(geometry<props> => Amenity<amenity_type> [name, access, amperage, backrest, beds, bottle, brand, brewery, building, capacity, cargo, colour, contact, covered, cuisine, date, delivery, denomination, description, diet, direction, drink, drinking_water, drive_through, emergency, fee, fuel, indoor, lit, material, network, opening_hours, operator, payment, phone, religion, seats, self_service, smoking, socket, voltage, website, wheelchair]);
}
